...
**Notes:** Validation monitor runs in a `threading.Thread` (not asyncio), checks every 2 seconds. `sfp_watch_loop` monitors for a Sweep-and-Flip pattern 10 minutes after exit.
...
**Notes:** Checks every 30 seconds. Cannot be blocked by scanning loops, DB hangs, or WS stalls. Soft at 15:32, hard SIGTERM at 15:40. `EOD_SOFT_SHUTDOWN=(15,32)`, `EOD_HARD_KILL=(15,40)`.
...
| (c) `eod_watchdog` hard SIGTERM | `eod_watchdog.py` [Bug 2A FIX NEW] | 15:40 IST if still alive |
...
[SHUTDOWN SEQUENCE — _cleanup_runtime()]
  ...
  1. focus_engine halts automatically (checks shutdown_event in trading_loop)
     └─ No new signals or orders accepted
